...
if __name__ == "__main__":
    ...
    parser.add_argument("--file", "-o", type=str, dest="output_file",
                        default="{}.flow".format(int(time.time())),
    ...
    try:
        ...
        for ts, client, export in get_export_packets(args.host, args.port):
            ...
            with open(args.output_file, "ab") as fh:  # open as append, not reading the whole file
                fh.write(line)

            elapsed_time = time.time() - start_time
            if elapsed_time >= 150:
                logger.info("Reached the time limit, exiting")
                break
